...
sub results_header {
    ...
    if ( $swr && ref $swr eq 'ARRAY' ) {
        $stopwords =
          @$swr > 1
          ? join( ', ', map { "<b>$_</b>" } @$swr )
          . ' are very common words and were not included in your search'
          : join( ', ', map { "<b>$_</b>" } @$swr )
...
'<font size="-1" face="Geneva, Arial, Helvetica, San-Serif">&nbsp;Page:</font>'
    ...
    $links .=
      qq[ <a href="$query_href&amp;start=$prev">Previous $prev_count</a>]
      if $prev_count;
    ...
    $links = qq[<tr><td colspan="2" bgcolor="#EEEEEE">$links</td></tr>]
      if $links;
...
sub show_result {
    ...
    my $title =
         $this_result->{$DocTitle}
      || $this_result->{swishdocpath}
      || '?';
...
sub get_index_select_list {
    ...
    return ''
      unless exists $select_config->{labels}
    ...
    my @cols =
      $select_config->{columns}
      ? ( '-columns', $select_config->{columns} )
      : ();

    return join "\n", '<br>', ( $select_config->{description} || 'Select: ' ),
      $q->$method(
        -name    => 'si',
        -values  => [ 0 .. $#labels ],
        -default => 0,
